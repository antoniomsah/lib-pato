...
struct point {
	...
	bool operator<(const point& rhs) const { return x < rhs.x - EPS or (abs(x-rhs.x) < EPS and y < rhs.y - EPS); }
	bool operator>(const point& rhs) const { return rhs < *this; }
	bool operator==(const point& rhs) const { return abs(x-rhs.x) < EPS and abs(y-rhs.y) < EPS; }
	...
	bool is_perp(const point& rhs) const { return abs(*this * rhs) < EPS; }
...
struct line {
	...
	bool parallel(const line& rhs) const { return abs(v^rhs.v) < EPS; }
};
...
struct circle {
	...
	bool in(circle rhs) { return (c-rhs.c).norm() + r < rhs.r+EPS; }
	bool has(point p) { return (c-p).norm() < r+EPS; }
